class Juego {
  ...
  mostrar(aventura) {
    if (this.p && Array.isArray(this.p) && this.p.length > 12) {    //EL ARRAY PARA ALMACENAR COSAS 
      image(this.p[13], 0, 0, width, height);
    }
//CALCULAN EL TIEMPO DE JUEGO ANTES Y DURANTE 
...
//VERIFICO SI TODOS LOS PLANETAS ESTAN MUERTOS Y PASO DE ESTADO
  ...
  crearEnemigos() {
    this.enemigos = [];
    for (let i = 0; i < this.cantidadEnemigos; i++) {
      this.enemigos[i] = new Enemigo(i * 40 + 20, 100);     //UBI DE LOS PLANETAS
    }
  }
  ...
  controlarColisiones() {
    //CUANDO LA BALA TOCA EL PLANETA
    for (let i = this.personaje.balas.length - 1; i >= 0; i--) {
